      ...
      for (auto& rotation : shape.reduced_rotations) {
        ...
        for (size_t l = 0; l < size; ++l) {
          ...
          if (new_pd < cur_pd) {
            ...
            cur_pd = new_pd;
          } else if (layout.saEnabled) {
            // 模拟退火判断：是否接受恶化解
            double delta = CGAL::to_double(new_pd) - CGAL::to_double(cur_pd);
            // 计算接受概率 exp(-delta / T)
            double acceptance_prob = std::exp(-delta / layout.saTemperature);
            double random_val = rand::ud1(rand::random_engine3);
            if (random_val < acceptance_prob) {
              // 接受恶化解，保存分支点
              size_t branch_id = layout.save_branch_point(
                  idx, cur_pd, new_pd, shape.get_transform(), Point_2(point_x, point_y));
              shape.set(rotation, point_x, point_y);
              for (size_t k = 0; k < layout.poly_num - 1; k++) {
                if (k < idx) {
                  layout.set_pd(idx, k, temp[k]);
              ...
              cur_pd = new_pd;
              // 执行分支退火迭代
              bool improved = handle_branch_with_sa(layout, idx, cur_pd,
                                                    shape.get_transform(),
                                                    Point_2(point_x, point_y),
                                                    layout.saIterations);
              if (!improved) {
                // 退火未能改善，回滚
                layout.rollback_to_branch(branch_id);
              }
            }
          }
...
// 模拟退火单次迭代：对指定多边形尝试随机扰动
bool sa_iterate(Layout& layout, size_t idx, double temperature) {
  auto& shape = layout.sheet_parts[0][idx];
  FT cur_pd = layout.get_one_polygon_pd(idx);
  auto rotation = shape.get_rotation();

  // 获取当前变换参数
  auto ifr = geo::comp_ifr(layout.sheets[0].sheet, shape.transformed);
  auto bbox = ifr.bbox();

  // 生成随机扰动（在当前候选点附近小范围扰动）
  double perturbation_range = bbox.x_span() * 0.1;  // 10%的扰动范围
  double random_x = (rand::ud1(rand::random_engine3) - 0.5) * perturbation_range;
  double random_y = (rand::ud1(rand::random_engine3) - 0.5) * perturbation_range;

  auto cur_x = CGAL::to_double(shape.get_translate_ft_x());
  auto cur_y = CGAL::to_double(shape.get_translate_ft_y());
  auto new_x = cur_x + random_x;
  auto new_y = cur_y + random_y;

  // 检查新位置是否在边界内
  if (new_x < bbox.xmin() || new_x > bbox.xmax() ||
      new_y < bbox.ymin() || new_y > bbox.ymax()) {
    return false;
  }

  // 计算新位置的PD
  std::vector<FT> temp(layout.poly_num);
  for (size_t k = 0; k < layout.poly_num; k++) {
    if (k == idx) {
      temp[k] = FT(0);
      continue;
    ...
    auto& nfp = comp_nfp(shape_k.base, shape_k.get_rotation(),
                         shape_k.allowed_rotations, shape.base, rotation,
                         shape.allowed_rotations, layout);
    auto double_kx = shape_k.get_translate_double_x();
    auto double_ky = shape_k.get_translate_double_y();
    temp[k] = comp_pd(nfp, new_x - double_kx, new_y - double_ky, layout);
  }
  ...
  for (size_t k = 0; k < layout.poly_num; k++) {
    if (k == idx) continue;
    new_pd_ft += temp[k] * layout.get_miu(idx, k);
  }

  // Metropolis准则
  double delta = CGAL::to_double(new_pd_ft) - CGAL::to_double(cur_pd);
  if (delta < 0) {
    // 改善解，直接接受
...
// 处理分支的模拟退火迭代
bool handle_branch_with_sa(Layout& layout, size_t idx,
                           FT cur_pd,
                           const Transformation& start_trans,
                           const Point_2& start_pos,
                           size_t iterations) {
  double temperature = layout.saTemperature;
  bool improved = false;
  FT best_pd = cur_pd;

  for (size_t i = 0; i < iterations && temperature > 0.01; i++) {
    if (sa_iterate(layout, idx, temperature)) {
      auto new_pd = layout.get_one_polygon_pd(idx);
      if (new_pd < best_pd) {
        best_pd = new_pd;
        improved = true;
      }
    }
    temperature *= layout.saCoolingRate;
  }
